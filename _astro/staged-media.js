...
async function initializeStudio(container) {
  if (reducedMotionQuery.matches || container.dataset.initialized) return;
  ...
  const format = preferredFormat(preview);

  await loadVideo(preview, config.preview[format]);
  ...
  try {
    await loadVideo(full, config.full[format]);
...
if (studio) {
  const preloadObserver = new IntersectionObserver((entries, observer) => {
    ...
    observer.disconnect();
    void firstScroll.then(() => initializeStudio(studio));
  }, { rootMargin: "25% 0px" });
...
function createSequence(stage, frameCount) {
  ...
  async function preload() {
    ...
    const source = config[viewport()][preferredFormat(video)];
